...
class PGAgent(base_agent.BaseAgent):
    ...
    def _calc_return(self, r, done):
        ...
        for idx in reversed(range(r.shape[0])):
            if done[idx]:
                running_ret = 0.0
            running_ret = r[idx] + gamma * running_ret
            return_t[idx] = running_ret
    ...
    def _calc_adv(self, norm_obs, ret):
        ...
        with torch.no_grad():
            val_pred = self._model.eval_critic(norm_obs)
    ...
    def _calc_critic_loss(self, norm_obs, tar_val):
        ...
        val_pred = self._model.eval_critic(norm_obs).squeeze(-1)
        loss = torch.mean((val_pred - tar_val) ** 2)
        return loss
    ...
    def _calc_actor_loss(self, norm_obs, norm_a, adv):
        ...
        a_dist = self._model.eval_actor(norm_obs)
        log_prob = a_dist.log_prob(norm_a)
        adv_detached = adv.detach()
        loss = -torch.mean(log_prob * adv_detached)
        return loss